			...
			for key,val in tweet.items():
				try:
					...
					if image_url=='none':
						...
						if os.path.exists('data/images_nrv/%s.jpg'%val["id_str"]):
							#print("CONGRATS IMAGE EXISTS")
							continue
						else:
							urllib.request.urlretrieve(image_url, 'data/images_nrv/%s.jpg'%val["id_str"])
				except:
					continue
...
def start():
	...
	if os.listdir(image_path) == []:
		...
		os.system('python predicted_nrv.py')
		#<decide top5 classes from dataframe>
		dictionary_temporary={}
		list_ids=[]
		di_image_classes={}
		df=pd.read_csv("result_nrv.csv",delimiter=",")
		...
		#print(list_ids)
		for single_file in list_files_top_5_events:
		 	with open("data/non_radical_violence_events/top_5_events/%s"%single_file,"r") as f:
		 		for line in f:
		 			...
		 			for key,val in dict_event.items():
		 					if key in list_ids:
		 						...
		 						continue
		 	with open("data/non_radical_violence_events/top_5_events_images/%s"%single_file,"a") as f1:
