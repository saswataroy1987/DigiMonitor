...
import pymongo
import json
import clustering_RealTime_nve as CR
...
import summary_RealTime_nve as sR
import tweet_with_images_RealTime_nve as iR
import shutil
...
path='data/non_violence_extremism_events/Event_data_nve'


path_top_5_events='data/non_violence_extremism_events/top_5_events'
path_top_5_events_images='data/non_violence_extremism_events/top_5_events_images'
path_top_5_events_summary='data/non_violence_extremism_events/top_5_events_summary'

def remove2hour():
	print("i am under remove...................")
	#print("length of documents till now:  ",mydb.nve.count())
	sum_nve =0
	for i in range(5):
		mycol_nve=mydb[i]["nve"]
		sum_nve = sum_nve + mycol_nve.count()
	print("length of documents till now:  ",sum_nve)

	#cursor = mycol_nve.find(no_cursor_timeout=True)
	cursor = []
	for i in range(5):
		mycol_nve=mydb[i]["nve"]
	...
	for ele in cursor:
		try:
			...
			if tweet_time <= req_time:
				#result_del = mydb.nve.delete_one(ele)
				for i in range(5):
					mycol_nve=mydb[i]["nve"]
...
def incremental_clus(tweet):
	...
	if location:
		...
		for single_file in list_files:
			with open("data/non_violence_extremism_events/Event_data_nve/%s"%single_file,"r") as f:
				for line in f:
		...
		if max>= threshold:
			...
			for key,val in dict_similarity.items():
				if val==max:
					...
					with open("data/non_violence_extremism_events/Event_data_nve/%s"%key,"r") as f1:
						for line in f1:
					...
					#print(event_file)
					with open("data/non_violence_extremism_events/Event_data_nve/%s"%key,"w") as f2:
			...
			with open("data/non_violence_extremism_events/Event_data_nve/new_clus{0}.json".format(ijk),"w") as f3:
...
def removeTweets_after2hrs():
	...
	while True:
		...
		for i in range(5):
			mycol_nve=mydb[i]["nve"]
			sum_nve_initial = sum_nve_initial + mycol_nve.count()
		#if mydb.nve.count() > 0:
		if sum_nve_initial > 0:
			if os.listdir(path) == []:
			
				last_time=CR.clus()
				print("Clustering is done..now we wil delete test_nve.json")
				if os.path.exists("test_nve.json"):
					os.remove("test_nve.json")
				else:
				...
				for single_file in list_files_top_5_events:
					os.remove('data/non_violence_extremism_events/top_5_events/%s'%single_file)

				for single_file in list_files_event_images:
					os.remove('data/non_violence_extremism_events/top_5_events_images/%s'%single_file)

				for single_file in list_files_event_summary:
					os.remove('data/non_violence_extremism_events/top_5_events_summary/%s'%single_file)
				...
				for single_file in tuples_sorted:
					newPath = shutil.copy('data/non_violence_extremism_events/Event_data_nve/%s'%single_file[0], 'data/non_violence_extremism_events/top_5_events/%s'%single_file[0])	
				...
				for single_file in list_files:
					with open("data/non_violence_extremism_events/Event_data_nve/%s"%single_file,"r") as f:
						for line in f:
							single_file_di=json.loads(line)
							if len(single_file_di) == 0:
								os.remove("data/non_violence_extremism_events/Event_data_nve/%s"%single_file)
								...
								if not all_tweets:
									...
									os.rename("data/non_violence_extremism_events/Event_data_nve/%s"%single_file,"data/non_violence_extremism_events/Event_data_nve/%s.json"%event_name) 
		...
		if i == 4:
			...
			#if mydb.ve.count() > 0:
			if sum_nve  > 0:
				#print("Before deleting:....",mydb.ve.count())
				print("Before deleting:....",sum_nve)
				#remove2hour()
				for i in range(5):
					mycol_nve=mydb[i]["nve"]
					mycol_nve.remove({})
				#print("After deleting:....",mydb.ve.count())
				sum_nve_new =0
				for i in range(5):
					mycol_nve=mydb[i]["nve"]
					sum_nve_new = sum_nve_new + mycol_nve.count()
				print("After deleting:....",sum_nve_new)
			else:
