...
def clus():
	...
	#print("{0} no of tweets going to be clustered.....".format(mydb.nve.count()))
	sum_nve =0
	for i in range(5):
		mycol_nve=mydb[i]["nve"]
		sum_nve = sum_nve + mycol_nve.count()
	print("{0} no of tweets going to be clustered.....".format(sum_nve))
	...
	list_created_at=[]
	with open("test_nve.json","a",encoding="utf-8") as f:
		for ele in cursor:
			try:
				del ele['_id']
	...
	for i in range(0,len(list_clus_index)):
		...
		if len(dictionary_temporary) >0 :
			with open("data/non_violence_extremism_events/Event_data_nve/{0}.json".format(event_name),"a") as f:
