...
def clus():
	...
	#print("{0} no of tweets going to be clustered.....".format(mydb.nrv.count()))
	sum_nrv =0
	for i in range(5):
		mycol_nrv=mydb[i]["nrv"]
		sum_nrv = sum_nrv + mycol_nrv.count()
	print("{0} no of tweets going to be clustered.....".format(sum_nrv))
	...
	list_created_at=[]
	with open("test_nrv.json","a",encoding="utf-8") as f:
		for ele in cursor:
			try:
				del ele['_id']
	...
	for i in range(0,len(list_clus_index)):
		...
		if len(dictionary_temporary) >0 :
			with open("data/non_radical_violence_events/Event_data_nrv/{0}.json".format(event_name),"a") as f:
