...
import pymongo
import json
import clustering_RealTime_ve as CR
...
import summary_RealTime_ve as sR
import tweet_with_images_RealTime_ve as iR
import shutil
...
path='data/violence_extremism_events/Event_data_ve'


path_top_5_events='data/violence_extremism_events/top_5_events'
path_top_5_events_images='data/violence_extremism_events/top_5_events_images'
path_top_5_events_summary='data/violence_extremism_events/top_5_events_summary'

def remove2hour():
	print("i am under remove...................")
	#print("length of documents till now:  ",mydb.ve.count())

	sum_ve =0
	for i in range(5):
		mycol_ve=mydb[i]["ve"]
		sum_ve = sum_ve + mycol_ve.count()
	print("length of documents till now:  ",sum_ve)

	#cursor = mycol_ve.find(no_cursor_timeout=True)
	cursor = []
	for i in range(5):
		mycol_ve=mydb[i]["ve"]
	...
	for ele in cursor:
		try:
			...
			if tweet_time <= req_time:
				#result_del = mydb.ve.delete_one(ele)
				for i in range(5):
					mycol_ve=mydb[i]["ve"]
...
def incremental_clus(tweet):
	...
	if location:
		...
		for single_file in list_files:
			with open("data/violence_extremism_events/Event_data_ve/%s"%single_file,"r") as f:
				for line in f:
		...
		if max>= threshold:
			...
			for key,val in dict_similarity.items():
				if val==max:
					...
					with open("data/violence_extremism_events/Event_data_ve/%s"%key,"r") as f1:
						for line in f1:
					...
					#print(event_file)
					with open("data/violence_extremism_events/Event_data_ve/%s"%key,"w") as f2:
			...
			with open("data/violence_extremism_events/Event_data_ve/new_clus{0}.json".format(ijk),"w") as f3:
...
def removeTweets_after2hrs():
	...
	while True:
		...
		for i in range(5):
			mycol_ve=mydb[i]["ve"]
			sum_ve_initial = sum_ve_initial + mycol_ve.count()
		#if mydb.ve.count() > 0:
		if sum_ve_initial > 0:
			if os.listdir(path) == []:
			
				last_time=CR.clus()
				print("Clustering is done..now we wil delete test_ve.json")
				if os.path.exists("test_ve.json"):
					os.remove("test_ve.json")
				else:
				...
				for single_file in list_files_top_5_events:
					os.remove('data/violence_extremism_events/top_5_events/%s'%single_file)

				for single_file in list_files_event_images:
					os.remove('data/violence_extremism_events/top_5_events_images/%s'%single_file)

				for single_file in list_files_event_summary:
					os.remove('data/violence_extremism_events/top_5_events_summary/%s'%single_file)
				...
				for single_file in tuples_sorted:
					newPath = shutil.copy('data/violence_extremism_events/Event_data_ve/%s'%single_file[0], 'data/violence_extremism_events/top_5_events/%s'%single_file[0])	
				...
				for single_file in list_files:
					with open("data/violence_extremism_events/Event_data_ve/%s"%single_file,"r") as f:
						for line in f:
							single_file_di=json.loads(line)
							if len(single_file_di) == 0:
								os.remove("data/violence_extremism_events/Event_data_ve/%s"%single_file)
								...
								if not all_tweets:
									...
									os.rename("data/violence_extremism_events/Event_data_ve/%s"%single_file,"data/violence_extremism_events/Event_data_ve/%s.json"%event_name) 
		...
		if i == 4:
			...
			if sum_ve  > 0:
				...
				for i in range(5):
					mycol_ve=mydb[i]["ve"]
					sum_ve_new = sum_ve_new + mycol_ve.count()
				print("After deleting:....",sum_ve_new)
			else:
