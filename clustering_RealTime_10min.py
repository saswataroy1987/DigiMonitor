		...
		if location:
			for key in di:
				if key in location:
					...
					original=row["retweeted_status"]
					# print("original",original)
					if original is not None :
						id_virality= original["id_str"]
					else:
						id_virality= row["id_str"]
					list_tweets.append(row['preprocessed_text'])
					lis=row["entities"]["hashtags"]
					for e in lis:	
						hashtag.append(e["text"])
					dictionary_temporary[row["id_str"]]={"id_str":row["id_str"],"latitude":lat_long[0],"longitude":lat_long[1],"created_at":row["created_at"],"user_location":location,"text":row["text"],"main_event":0,"user_name":row["user"]["name"],"follower_count":row["user"]["followers_count"],"retweet_count":row["retweet_count"],"image_url":url,"id_virality":id_virality}
					break
	if len(hashtag)>=1:
...
def clus():
	...
	#print("{0} no of tweets going to be clustered.....".format(mydb.test2hour.count()))
	sum_10min =0
	for i in range(5):
		mycol10min=mydb[i]["test10min"]
		sum_10min = sum_10min + mycol10min.count()
	print("{0} no of tweets going to be clustered.....".format(sum_10min))
	...
	list_created_at=[]
	with open("test10min.json","a",encoding="utf-8") as f:
		for ele in cursor:
			try:
				del ele['_id']
	...
	for i in range(0,len(list_clus_index)):
		...
		if len(dictionary_temporary) >0 :
			with open("data/Event_data_10min/{0}.json".format(event_name),"a") as f:
