...
import pymongo
import json
import clustering_RealTime_rv as CR
...
import summary_RealTime_rv as sR
import tweet_with_images_RealTime_rv as iR
import shutil
...
path='data/radical_violence_events/Event_data_rv'


path_top_5_events='data/radical_violence_events/top_5_events'
path_top_5_events_images='data/radical_violence_events/top_5_events_images'
path_top_5_events_summary='data/radical_violence_events/top_5_events_summary'

def remove2hour():
	print("i am under remove...................")
	#print("length of documents till now:  ",mydb.rv.count())
	sum_rv =0
	for i in range(5):
		mycol_rv=mydb[i]["rv"]
		sum_rv = sum_rv + mycol_rv.count()
	print("length of documents till now:  ",sum_rv)
	...
	for ele in cursor:
		try:
			...
			if tweet_time <= req_time:
				#result_del = mydb.rv.delete_one(ele)
				for i in range(5):
					mycol_rv=mydb[i]["rv"]
...
def incremental_clus(tweet):
	...
	if location:
		...
		for single_file in list_files:
			with open("data/radical_violence_events/Event_data_rv/%s"%single_file,"r") as f:
				for line in f:
		...
		if max>= threshold:
			...
			for key,val in dict_similarity.items():
				if val==max:
					...
					with open("data/radical_violence_events/Event_data_rv/%s"%key,"r") as f1:
						for line in f1:
					...
					#print(event_file)
					with open("data/radical_violence_events/Event_data_rv/%s"%key,"w") as f2:
			...
			with open("data/radical_violence_events/Event_data_rv/new_clus{0}.json".format(ijk),"w") as f3:
...
def removeTweets_after2hrs():
	...
	while True:
		...
		for i in range(5):
			mycol_rv=mydb[i]["rv"]
			sum_rv_initial = sum_rv_initial + mycol_rv.count()
		#if mydb.rv.count() > 0:
		if sum_rv_initial > 0:
			if os.listdir(path) == []:
			
				last_time=CR.clus()
				print("Clustering is done..now we wil delete test_rv.json")
				if os.path.exists("test_rv.json"):
					os.remove("test_rv.json")
				else:
				...
				for single_file in list_files_top_5_events:
					os.remove('data/radical_violence_events/top_5_events/%s'%single_file)

				for single_file in list_files_event_images:
					os.remove('data/radical_violence_events/top_5_events_images/%s'%single_file)

				for single_file in list_files_event_summary:
					os.remove('data/radical_violence_events/top_5_events_summary/%s'%single_file)
				...
				for single_file in tuples_sorted:
					newPath = shutil.copy('data/radical_violence_events/Event_data_rv/%s'%single_file[0], 'data/radical_violence_events/top_5_events/%s'%single_file[0])	
				...
				for single_file in list_files:
					with open("data/radical_violence_events/Event_data_rv/%s"%single_file,"r") as f:
						for line in f:
							single_file_di=json.loads(line)
							if len(single_file_di) == 0:
								os.remove("data/radical_violence_events/Event_data_rv/%s"%single_file)
								...
								if not all_tweets:
									...
									os.rename("data/radical_violence_events/Event_data_rv/%s"%single_file,"data/radical_violence_events/Event_data_rv/%s.json"%event_name) 
		...
		if i == 4:
			...
			if sum_rv > 0:
				...
				for i in range(5):
					mycol_rv=mydb[i]["rv"]
					sum_rv_new = sum_rv_new + mycol_rv.count()
				print("After deleting:....",sum_rv_new)
			else:
