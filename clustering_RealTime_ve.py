...
def clus():
	...
	#print("{0} no of tweets going to be clustered.....".format(mydb.ve.count()))
	sum_ve =0
	for i in range(5):
		mycol_ve=mydb[i]["ve"]
		sum_ve = sum_ve + mycol_ve.count()
	print("{0} no of tweets going to be clustered.....".format(sum_ve))
	...
	list_created_at=[]
	with open("test_ve.json","a",encoding="utf-8") as f:
		for ele in cursor:
			try:
				del ele['_id']
	...
	for i in range(0,len(list_clus_index)):
		...
		if len(dictionary_temporary) >0 :
			with open("data/violence_extremism_events/Event_data_ve/{0}.json".format(event_name),"a") as f:
